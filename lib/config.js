...
/**
 * Try loading a module first as CJS (with `require`) and then as ESM
 * (with `import`).  In the case of ESM, a promise for the module will
 * be returned.
 *
 * @param {string} modulePath Path of the module to import.
 * @returns {Object|Promise<Object>}
 */
function requireOrImport (modulePath) {
  try {
 ...
 *
 * @param {Object|Promise<Object>} configModule The config module or a promise for it.
 * @param {Object} config The config module or a promise for it.
 * @returns {Config|Promise<Config>}
 */
function processConfig (configModule, config) {
  if (typeof configModule === 'function') {
    return configModule(config)
  }
  if (configModule && typeof configModule.then === 'function') {
    return configModule.then(function (mod) {
...
function parseConfig (configFilePath, cliOptions, parseOptions) {
  ...
  if (configFilePath) {
    try {
      ...
      configModule = requireOrImport(configFilePath)
  ...
  try {
    configModuleReturn = processConfig(configModule, config)
  } catch (e) {
    return fail('Error in config file!\n', e)
  }
